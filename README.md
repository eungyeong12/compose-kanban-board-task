...
- [ ] data, domain, ui로 패키지를 분리한다.
- [ ] data 계층에 `TasksData` 와 `TasksRepository` 를 생성하여 데이터를 저장하거나 가져오는 로직을 분리한다.
- [ ] ui 계층의 비즈니스 로직을 `HomeViewModel` 로 분리한다.

### 2.  단위 테스트를 작성한다.
- [ ] 제목이 비어 있거나 공백만 있는 경우 생성이 불가능하다.
- [ ] 담당자가 비어 있거나 공백만 있는 경우 생성이 불가능하다.
- [ ] 태그 내용이 비어있는 경우 생성이 불가능하다.
- [ ] 태그의 길이가 5자를 초과할 경우 생성이 불가능하다.
- [ ] 태그가 5개를 초과할 경우 생성이 불가능하다.
